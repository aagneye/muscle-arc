...
FILE="${1:-submissions/submission_v8.csv}"
MSG="${2:-v8 letterbox+Tversky phase4 + extrapolated geometry + Radon PA ensemble}"
...
sleep 35
bash /tmp/vm_list_scores.sh 2>/dev/null || true
...
[print(x.get('date') or x.get('dateSubmitted'),'|',x.get('publicScore'),'|',x.get('status'),'|',(x.get('description') or '')[:90]) for x in items[:6]]"
