...
FILE="${1:-submissions/submission_v5.csv}"
MSG="${2:-v5 fasc_v2 Tversky retrain + calibrated dual-scale}"
...
[print(x.get('date') or x.get('dateSubmitted'), x.get('publicScore'), x.get('status'), (x.get('description') or '')[:80]) for x in items[:5]]"
